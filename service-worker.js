// 🎭 エモート所持率管理 - Service Worker（PWAオフライン対応）
// ────────────────────────────────────────────────────────────
...
const CACHE_VERSION = 'v15'; // v15: 連鎖オーバーレイのフォーカストラップ誤復元バグを修正（pfSuppressTrapFocusRestore追加）
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(
      keys.filter((key) => key.startsWith('emote-shell-') && key !== CACHE_NAME)
